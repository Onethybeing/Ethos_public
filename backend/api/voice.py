...
Endpoints:
  GET /article/{article_id}/voice?mode=anchor|podcast|drama
    — LLM generates a ~100-word spoken script for the article.
      TTS is handled client-side via the browser Web Speech API.
      Returns JSON: { script, mode, label }
"""
...
async def get_voice(
    ...
    """
    Generate a spoken script for an article.
    TTS playback is handled client-side via the Web Speech API.

    Returns:
        { "script": "...", "mode": "anchor", "label": "News Anchor" }
    """
